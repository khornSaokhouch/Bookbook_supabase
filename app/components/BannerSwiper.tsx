  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      const { data, error } = await supabase
        ...
        .gte("end_date", now) // Get ongoing or upcoming events
        .order("start_date", { ascending: true }); // Optional: soonest events first
  ...
  if (loading) {
    return (
      <div className="relative w-full h-[400px] bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 overflow-hidden">
        ...
        <div className="absolute top-20 left-10 w-32 h-32 bg-white/5 rounded-full blur-2xl animate-bounce" />
        <div className="absolute bottom-20 right-10 w-24 h-24 bg-white/10 rounded-full blur-xl animate-pulse" />
        ...
        {events.map((event, index) => (
          <SwiperSlide key={event.event_id}>
            <div className="relative w-full h-full group overflow-hidden">
              ...
              <div className="absolute top-10 right-10 w-20 h-20 bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-full blur-2xl animate-pulse" />
              <div className="absolute bottom-20 left-10 w-32 h-32 bg-gradient-to-r from-blue-500/10 to-purple-500/10 rounded-full blur-3xl" />
