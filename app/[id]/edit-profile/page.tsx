...
const EditProfile = () => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (fetchedUser) {
          ...
          setError("User profile not found."); // Clearer message
        }
      } catch (e: any) {
  ...
  const handleConfirmUpdate = async () => {
    ...
    try {
      // Make sure you're updating the user with the correct user_id
      ...
      if (result.success) {
        ...
        if (updatedProfile) {
          setUser(updatedProfile); // Update the state with the *new* profile
        } else {
  ...
  const profileCardTransitions = {
    ...
    animate: { scale: 1, opacity: 1, y: 0, transition: { duration: 0.5, ease: "easeOut", delay: 0.2 } },
  };

  const avatarTransitions = {
    initial: { scale: 0, opacity: 0 },
    animate: { scale: 1, opacity: 1, transition: { type: "spring", stiffness: 100, damping: 15, delay: 0.4 } },
  };

  const formElementTransitions = {
    initial: { opacity: 0, y: 10 },
    animate: { opacity: 1, y: 0, transition: { duration: 0.4, ease: "easeOut", staggerChildren: 0.1, delayChildren: 0.6 } },
  };
      ...
      <AnimatePresence>
        {successMessage && (
          ...
          >
            ...
            <button onClick={handleCloseAlert} className="text-xl font-semibold">
              ×
            </button>
          </motion.div>
        )}
              ...
              <motion.img
                src={
                  user?.image_url
                    ? supabase.storage
                        .from("image-user")
                        .getPublicUrl(user.image_url).data.publicUrl
                    : "/default-profile.png"
                }
                alt="User Avatar"
